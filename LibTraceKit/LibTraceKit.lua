...
local function cfn(colorRGB)
  ...
  assertsafe(c and type(c.WrapTextInColorCode) == 'function',
...
function TraceKit:__Init(namespace, hexColor)
  assertsafe(type(namespace) == 'string' and #strtrim(namespace) > 0,
    'Invalid namespace: <%s>. TraceKit:__Init(namespace): <namespace> should be a non-empty string',
    tostring(namespace))
...
function TraceKit:WithTag(tag)
  assertsafe(type(tag) == 'string' and #strtrim(tag) > 0,
    'Invalid tag: <%s>. TraceKit:SetTag(tag): <tag> should be a non-empty string.',
    tostring(tag))
...
function TraceKit:WithDelimiter(delim)
  assertsafe(type(delim) == 'string',
    'Invalid delim: <%s>. TraceKit:SetDelimiter(delim): <delim> should be a string.',
    tostring(delim))
  self.delim = delim
  return self, self.formatter
end
